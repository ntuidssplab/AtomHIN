...
class PSHGCN(nn.Module):
    ...
    def __init__(
        self, emb_dim, hidden_x, hidden_l, nclass, in_dims: dict[NType, int],
        label_keys, layers_x, layers_l, coe_num, expander, dropout, input_drop,
        bias=False
    ):
        super().__init__()
        ...
        for ntype, dim in in_dims.items():
            self.embedings_x[ntype] = MLP(
                dim, emb_dim, emb_dim, self.layers_emb, dropout, bias=bias
            )
        # for node in node_types:
        #     if node == "P":
        #         self.embedings_x[node] = MLP(
        #             128, emb_dim, emb_dim, self.layers_emb, dropout, bias=bias
        #         )
        #     else:
        #         self.embedings_x[node] = MLP(
        #             256, emb_dim, emb_dim, self.layers_emb, dropout, bias=bias
        #         )

        self.embedings_l = nn.ParameterDict({})
        for key in label_keys:
            self.embedings_l[key] = MLP(
                nclass, emb_dim, emb_dim, self.layers_emb, dropout, bias=bias
            )

        self.lin_residue = nn.Linear(in_dims['P'], emb_dim, bias=False)
