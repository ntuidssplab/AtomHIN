...
class LMSPS(nn.Module):
    ...
    def __init__(
        self, dataset, data_size, nfeat, hidden, nclass, num_feats,
        num_label_feats, tgt_key, dropout, input_drop, att_drop, label_drop,
        n_layers_2, n_layers_3, residual=False, bns=False, label_bns=False,
        ...
        self.feat_project_layers = nn.Sequential(
            Conv1d1x1(
                nfeat, hidden, self.num_meta, bias=True,
            ...
            nn.Dropout(dropout),
            Conv1d1x1(
                hidden, hidden, self.num_meta, bias=True,
                cformat='channel-first'
            ),
            nn.LayerNorm([self.num_meta, hidden]),
            nn.PReLU(),
        ...
        if num_label_feats > 0 and label_path:
            self.label_feat_project_layers = nn.Sequential(
                Conv1d1x1(
                    nclass, hidden, self.num_label_meta, bias=True,
                    cformat='channel-first'
                ),
                ...
                nn.Dropout(dropout),
                Conv1d1x1(
                    hidden, hidden, self.num_label_meta, bias=True,
                    cformat='channel-first'
                ),
                nn.LayerNorm([self.num_label_meta, hidden]),
                nn.PReLU(),
        ...
        self.concat_project_layer = nn.Linear(
            (self.num_meta + self.num_label_meta) * hidden, hidden
        )
        ...
        lr_output_layers = [
            ...
            add_nonlinear_layers(hidden, dropout, bns)
    ...
    def reset_parameters(self):
        ...
        if self.dataset != 'products':
            nn.init.xavier_uniform_(
                self.concat_project_layer.weight, gain=gain
            )
    ...
    def forward(self, feats_dict, layer_feats_dict, label_emb):
        ...
        if self.tgt_key not in self.path:
            feats_dict = {
                k: v
                for k, v in feats_dict.items() if k != self.tgt_key
            }
        ...
        if self.label_feat_project_layers is not None:
            label_feats = self.input_drop(
                torch.stack(list(layer_feats_dict.values()), dim=1)
            )
...
class LMSPS_Se(nn.Module):
    ...
    def __init__(
        self, dataset, data_size, hidden, nclass, num_feats, num_label_feats,
        label_feat_keys, tgt_key, dropout, input_drop, label_drop, device,
        residual=False, label_residual=True, num_sampled=0
    ):
        ...
        self.lr_output = nn.Sequential(
            nn.Linear(hidden, nclass, bias=False), nn.BatchNorm1d(nclass)
        )
    ...
    def forward(self, epoch_sampled, feats_dict, label_feats_dict, label_emb):
        ...
        meta_path_sampled = [
            all_meta_path[i] for i in range(self.num_feats)
            if i in epoch_sampled
        ]
        label_meta_path_sampled = [
            all_meta_path[i] for i in range(self.num_feats, self.num_paths)
            if i in epoch_sampled
        ]
        ...
        for k, v in label_feats_dict.items():
            if k in self.labels_embeding and k in label_meta_path_sampled:
                label_feats_dict[k] = self.input_drop(
                    v @ self.labels_embeding[k]
                )

        x = [feats_dict[k] for k in meta_path_sampled
             ] + [label_feats_dict[k] for k in label_meta_path_sampled]
    ...
    def sample(self, keys, label_keys, lam, topn, all_path=False):
        ...
        seq_softmax = None if self.alpha is None else F.softmax(
            self.alpha, dim=-1
        )
        ...
        if topn:
            ...
            id_paths = [
                k for k in range(length) if seq_softmax[k].item() >= threshold
            ]
        path = [keys[i] for i in range(len(keys)) if i in id_paths]
        label_path = [
            label_keys[i] for i in range(len(label_keys))
            if i + len(keys) in id_paths
        ]
