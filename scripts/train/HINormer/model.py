"""Credit: https://github.com/Ffffffffire/HINormer"""
...
class REConv(nn.Module):
    ...
    def __init__(
        self, in_feats, out_feats, norm='both', num_type=4, weight=True,
    ...
    def forward(self, graph, feat, type_info):
        with graph.local_scope():
            ...
            graph.srcdata[
                'h'] = feat * self.weight_type[type_info].reshape(-1, 1)
...
class AGTLayer(nn.Module):
    ...
    def __init__(
        self, embeddings_dimension, nheads=2, att_dropout=0.5, emb_dropout=0.5,
    ...
    def forward(self, h, rh=None):
        batch_size = h.size()[0]
        fl = self.linear_l(h).reshape(
            batch_size, -1, self.nheads, self.head_dim
        ).transpose(1, 2)
        fr = self.linear_r(h).reshape(
            batch_size, -1, self.nheads, self.head_dim
        ).transpose(1, 2)

        score = self.att_l(self.leaky(fl)) + self.att_r(self.leaky(fr)
                                                        ).permute(0, 1, 3, 2)

        if rh is not None:
            r_k = self.r_source(rh).reshape(
                batch_size, -1, self.nheads, self.rl_dim
            ).transpose(1, 2)
            r_q = self.r_target(rh).reshape(
                batch_size, -1, self.nheads, self.rl_dim
            ).permute(0, 2, 3, 1)
            score_r = r_k @ r_q
...
class HINormer(nn.Module):
    ...
    def __init__(
        self, g, num_class, input_dimensions, embeddings_dimension=64,
        num_layers=8, num_gnns=2, nheads=2, dropout=0, temper=1.0, num_type=4,
        beta=1
    ):
        ...
        self.fc_list = nn.ModuleList(
            [
                nn.Linear(in_dim, embeddings_dimension)
                for in_dim in input_dimensions
            ]
        )
        ...
        for layer in range(self.num_gnns):
            self.GCNLayers.append(
                GraphConv(
                    self.embeddings_dimension, self.embeddings_dimension,
                    activation=F.relu
                )
            )
            self.RELayers.append(
                REConv(
                    num_type, num_type, activation=F.relu, num_type=num_type
                )
            )
        for layer in range(self.num_layers):
            self.GTLayers.append(
                AGTLayer(
                    self.embeddings_dimension, self.nheads, self.dropout,
                    self.dropout, temper=temper, rl=True, rl_dim=num_type,
                    beta=beta
                )
            )
    ...
    def forward(self, features_list, seqs, type_emb, node_type, norm=False):
        ...
        if isinstance(seqs, dict):
            output = {}
            for key, seqs_ in seqs.items():
                h = gh[seqs_]
                for layer in range(self.num_layers):
                    h = self.GTLayers[layer](h, rh=r[seqs_])
                output[key] = self.Prediction(h[:, 0, :])
                if norm:
                    output[key] = output[key] / (
                        torch.norm(output[key], dim=1, keepdim=True) + 1e-12
                    )
            return output
        h = gh[seqs]
        r = r[seqs]
