# Credit: https://github.com/ICT-GIMLab/SeHGNN/tree/master
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        f = self.query(x).view(B, M, H,
                               -1).permute(0, 2, 1, 3)  # [B, H, M, -1]
        g = self.key(x).view(B, M, H, -1).permute(0, 2, 3, 1)  # [B, H, -1, M]
        h = self.value(x).view(B, M, H,
                               -1).permute(0, 2, 1, 3)  # [B, H, M, -1]
...
class LinearPerMetapath(nn.Module):
    ...
    def __init__(self, cin, cout, num_metapaths):
        ...
        self.W = nn.Parameter(
            torch.randn(self.num_metapaths, self.cin, self.cout)
        )
...
class SeHGNN(nn.Module):
    ...
    def __init__(
        self, dataset, nfeat, hidden, nclass, feat_keys, label_feat_keys,
        tgt_type, dropout, input_drop, att_drop, n_fp_layers, n_task_layers,
        act, residual=False, data_size=None, drop_metapath=0., num_heads=1
    ):
        super().__init__()
        ...
        if self.dataset not in ['IMDB', 'Freebase']:
            self.task_mlp = nn.Sequential(
                *(
                    [
                        ...
                        nn.BatchNorm1d(
                            nclass, affine=False, track_running_stats=False
                        )
                    ]
                )
            )
            ...
            self.task_mlp = nn.ModuleList(
                [nn.Sequential(nn.PReLU(), nn.Dropout(dropout))] + [
                    nn.Sequential(
                        nn.Linear(hidden, hidden),
                        nn.BatchNorm1d(hidden, affine=False), nn.PReLU(),
                        nn.Dropout(dropout)
                    ) for _ in range(n_task_layers - 1)
                ] + [
                    nn.Sequential(
