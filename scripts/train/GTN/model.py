# Credit: https://github.com/BUPT-GAMMA/OpenHGNN/tree/main/openhgnn/output/GTN
import dgl
...
def transform_relation_graph_list(hg, category, identity=True):
    ...
    for i in range(num_edge_type + 1):
        ...
        sg = dgl.graph(
            (edges[0][e_ids], edges[1][e_ids]), num_nodes=g.num_nodes()
        )
...
class fastGTN(nn.Module):
    ...
    def build_model_from_args(cls, args, hg):
        ...
        # add self-loop edge
        return cls(
            num_edge_type=num_edge_type, num_channels=args.num_channels,
            in_dim=args.hidden_dim, hidden_dim=args.hidden_dim,
            num_class=args.out_dim, num_layers=args.num_layers,
            category=args.category, norm=args.norm_emd_flag,
            identity=args.identity
        )

    def __init__(
        self, num_edge_type, num_channels, in_dim, hidden_dim, num_class,
        num_layers, category, norm, identity
    ):
        ...
        self.linear1 = nn.Linear(
            self.hidden_dim * self.num_channels, self.hidden_dim
        )
    ...
    def forward(self, hg, h):
        with hg.local_scope():
            ...
            for i in range(self.num_layers):
                ...
                for n_c in range(self.num_channels):
                    edge_weight = self.norm(
                        hat_A[n_c], hat_A[n_c].edata['w_sum']
                    )
                    H[n_c] = self.dropout(
                        self.gcn(hat_A[n_c], H[n_c], edge_weight=edge_weight)
                    )
            X_ = self.linear1(th.cat(H, dim=1))
            X_ = F.relu(X_)
            y = self.linear2(self.dropout(X_))
