...
data_url = {
    'amazon':
    'https://cloud.tsinghua.edu.cn/d/10974f42a5ab46b99b88/files/?p=%2Famazon_ini.zip&dl=1',
    'LastFM':
    'https://cloud.tsinghua.edu.cn/d/10974f42a5ab46b99b88/files/?p=%2FLastFM_ini.zip&dl=1',
    'PubMed':
    'https://cloud.tsinghua.edu.cn/d/10974f42a5ab46b99b88/files/?p=%2FPubMed_ini.zip&dl=1'
}
...
class HGBDataLoader:
    ...
    def __init__(self, path, edge_types=[]):
        ...
        if os.path.exists(path):
            ...
            os.system(
                "unzip {} -d {}".format(
                    path + '.zip', os.path.join(*path.split('/')[:-1])
                )
            )
    ...
    def get_sub_graph(self, node_types_tokeep):
        ...
        for edge_type in self.links['count']:
            ...
            if h in keep and t in keep:
                ...
                new_links['data'][net] = self.links['data'][et][old_idx
                                                                ][:, old_idx]
                new_edge_type += 1
    ...
    def dfs(self, now, meta, meta_dict):
        ...
        th_node = now[-1]
        for col in self.re_cache[meta[0]
                                 ][th_node]:  # th_mat[th_node].nonzero()[1]:
    ...
    def get_full_meta_path(self, meta=[], symmetric=False):
        ...
        if len(meta) == 1:
            meta_dict = {}
            start_node_type = self.links['meta'][
                meta[0]][0] if meta[0] >= 0 else self.links['meta'][-meta[0] -
                                                                    1][1]
            trav = range(
            ...
            meta2 = meta[mid:]
            start_node_type = self.links['meta'][
                meta1[0]][0] if meta1[0] >= 0 else self.links['meta'][-meta1[0]
                                                                      - 1][1]
            trav = range(
                self.nodes['shift'][start_node_type],
                self.nodes['shift'][start_node_type] +
            ...
            start_node_type = self.links['meta'][
                meta2[0]][0] if meta2[0] >= 0 else self.links['meta'][-meta2[0]
                                                                      - 1][1]
            trav = range(
                self.nodes['shift'][start_node_type],
                self.nodes['shift'][start_node_type] +
    ...
    def gen_file_for_evaluate(
        self, edge_list, confidence, edge_type, file_path, flag
    ):
        """
    ...
    def list_to_sp_mat(self, li):
        ...
        return sp.coo_matrix(
            (data, (i, j)), shape=(self.nodes['total'], self.nodes['total'])
        ).tocsr()
    ...
    def get_test_neigh(self):
        random.seed(1)
        neg_neigh, pos_neigh, test_neigh, test_label = dict(), dict(), dict(
        ), dict()
        ...
        for r_id in self.links_test['data'].keys():
            pos_links += self.links_test['data'][r_id] + self.links_test[
                'data'][r_id].T
        for r_id in self.valid_pos.keys():
            values = [1] * len(self.valid_pos[r_id][0])
            valid_of_rel = sp.coo_matrix(
                (values, self.valid_pos[r_id]), shape=pos_links.shape
            )
        ...
        r_double_neighs = sp.coo_matrix(
            (
                data[pos_count_index],
                (row[pos_count_index], col[pos_count_index])
        ...
        relation_range = [
            self.nodes['shift'][k] for k in range(len(self.nodes['shift']))
        ] + [self.nodes['total']]
        for r_id in self.links_test['data'].keys():
            neg_neigh[r_id] = defaultdict(list)
            h_type, t_type = self.links_test['meta'][r_id]
            r_id_index = np.where(
                (row >= relation_range[h_type])
                & (row < relation_range[h_type + 1])
                & (col >= relation_range[t_type])
                & (col < relation_range[t_type + 1])
            )[0]
            # r_num = np.zeros((3, 3))
            # for h_id, t_id in zip(row, col):
        ...
        for r_id in edge_types:
            ...
            for h_id in sorted(list(pos_neigh[r_id].keys())):
                ...
                neg_list = random.choices(
                    neg_neigh[r_id][h_id], k=len(pos_list[0])
                ) if len(neg_neigh[r_id][h_id]) != 0 else []
    ...
    def get_test_neigh_w_random(self):
        ...
        for r_id in edge_types:
            ...
            pos_neigh[r_id], neg_neigh[r_id] = defaultdict(list), defaultdict(
                list
            )
            (row, col), data = self.links_test['data'][r_id].nonzero(
            ), self.links_test['data'][r_id].data
    ...
    def load_nodes(self):
        ...
        with open(
            os.path.join(self.path, 'node.dat'), encoding='utf-8'
        ) as f:
            for line in f:
                th = line.split('\t')
                if len(th) == 4:
