...
def create_products_of_givens_rotations(dim, seed):
    ...
    for _ in range(nb_givens_rotations):
        ...
        slice_i = q[index_i]
        slice_j = q[index_j]
        new_slice_i = math.cos(random_angle
                               ) * slice_i + math.cos(random_angle) * slice_j
        new_slice_j = -math.sin(random_angle
                                ) * slice_i + math.cos(random_angle) * slice_j
...
def relu_kernel_transformation(
    ...
    if projection_matrix is None:
        ...
        data_dash = ratio * torch.einsum(
            "bnhd,md->bnhm", data, projection_matrix
        )
...
def denominator_gumbel(qs, ks):
    all_ones = torch.ones([ks.shape[0]]).to(qs.device)
    ks_sum = torch.einsum(
        "nbhkm,n->bhkm", ks, all_ones
    )  # ks_sum refers to O_k in the paper
    return torch.einsum("nbhm,bhkm->nbhk", qs, ks_sum)


def kernelized_softmax(
    query, key, value, kernel_transformation, projection_matrix=None,
    edge_index=None, tau=0.25, return_weight=True
):
    '''
...
def kernelized_gumbel_softmax(
    query, key, value, kernel_transformation, projection_matrix=None,
    edge_index=None, K=10, tau=0.25, return_weight=True
):
    '''
    fast computation of all-pair attentive aggregation with linear complexity
    input: query/key/value [B, N, H, D]
    ...
    gumbels = (
        -torch.empty(
            key_prime.shape[:-1] +
            (K, ), memory_format=torch.legacy_contiguous_format
        ).exponential_().log()
    ).to(query.device) / tau  # [N, B, H, K]
    key_t_gumbel = key_prime.unsqueeze(3) * gumbels.exp().unsqueeze(
        4
    )  # [N, B, H, K, M]
    z_num = numerator_gumbel(
        query_prime, key_t_gumbel, value
    )  # [N, B, H, K, D]
...
class NodeFormerConv(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, num_heads,
        kernel_transformation=softmax_kernel_transformation,
        projection_matrix_type='a', nb_random_features=10, use_gumbel=True,
        nb_gumbel_sample=10, rb_order=0, rb_trans='sigmoid', use_edge_loss=True
    ):
    ...
    def forward(self, z, adjs, tau):
        ...
        if self.use_gumbel and self.training:  # only using Gumbel noise for training
            ...
            z_next, weight = kernelized_softmax(
                query, key, value, self.kernel_transformation,
                projection_matrix, adjs[0], tau, self.use_edge_loss
            )
        ...
        for i in range(self.rb_order):
            z_next += add_conv_relational_bias(
                value, adjs[i], self.b[i], self.rb_trans
            )
...
class BaseNodeFormer(nn.Module):
    '''
    ...
    def __init__(
        ...
        kernel_transformation=softmax_kernel_transformation,
        nb_random_features=30, use_bn=True, use_gumbel=True, use_residual=True,
        use_act=False, use_jk=False, nb_gumbel_sample=10, rb_order=0,
...
class NodeFormer(BaseNodeFormer):
    """A simple extended version that have adapters to accept heterogeneous graph as input
    """

    def __init__(
        self,
        in_channels: dict[NType, int],
        hidden_channels,
        out_channels,
        num_layers=2,
        num_heads=4,
        ...
        super().__init__(
            hidden_channels, hidden_channels, out_channels, num_layers,
            num_heads, dropout, kernel_transformation, nb_random_features,
            use_bn, use_gumbel, use_residual, use_act, use_jk,
            nb_gumbel_sample, rb_order, rb_trans, use_edge_loss
        )

        self.proj = nn.ModuleDict(
            {
                ntype: nn.Linear(in_c, hidden_channels)
                for ntype, in_c in in_channels.items()
            }
        )
        return

    def forward(self, x: dict[NType, torch.Tensor], adjs, tau=1.0):

        hs = [proj(x[ntype]) for ntype, proj in self.proj.items()]
        hs = torch.concatenate(hs, dim=0)
        return super().forward(hs, adjs, tau=tau)
