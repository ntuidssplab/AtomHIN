...
# pylint: disable= no-member, arguments-differ, invalid-name, inconsistent-quotes
...
class SimpleHGNConv(nn.Module):
    ...
    def set_allow_zero_in_degree(self, set_value):
        # never reached, idk what is this
        self._allow_zero_in_degree = set_value

    def forward(self, graph, feat, e_feat, res_attn=None, e_weight=None):
        #print("feat=",feat)
        #print("e_feat=",e_feat)
        with graph.local_scope():
            ...
            if e_weight is not None:
                if isinstance(e_weight, str):
                    e_weight = graph.edata[e_weight]
                a *= e_weight.view(-1, 1, 1)
            graph.edata['a'] = a
            ...
            if self.residual is not None:
                resval = self.residual(feat).view(
                    feat.shape[0], -1, self._out_feats
                )
                out = out + resval
    ...
    def __repr__(self):
        return (
            self.__class__.__name__ + " (" + str(self._in_feats) + " -> " +
            str(self._out_feats) + f", num_heads={self._num_heads}"
            ")"
        )
