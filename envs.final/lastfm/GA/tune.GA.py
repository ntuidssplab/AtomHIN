...
from dhgl.data.link_prediction.lastfm import LastFMDataset
...
BASE_ENV_FILE = os.path.abspath(
    os.path.join(__file__, '..', '_lastfm.sRGCN.env')
)
assert os.path.isfile(BASE_ENV_FILE)
CACHE_DIR = os.path.abspath(
    os.path.join(__file__, '..', '_lastfm.sRGCN.tune-cache')
)

NTYPES = LastFMDataset.ntypes
...
space['num_layers'] = tune.randint(0, 13)
...
POINTS_TO_EVALUATE = [
    { # Vanilla
        ...
        'tag': 1,
        'num_layers': 2,
        'user-artist|artist-user': 0,
        'user-user': 0,
        'artist-tag|tag-artist': 0,
    },
    { # Vanilla
        'user': 1,
        'artist': 1,
